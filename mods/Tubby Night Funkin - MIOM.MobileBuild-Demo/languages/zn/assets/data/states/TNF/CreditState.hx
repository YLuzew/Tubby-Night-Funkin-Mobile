...
function create() {
    ...
    // 在 create() 中创建 heart 后，设置缩放中心为图片中心，然后对 scale.x 进行 PingPong 动画
    ...
    heart.centerOrigin(); // 关键：让缩放围绕图片中心，而不是左上角
    // 无限循环水平挤压（scale.x 从 0.79 到 0 再回到 0.79）
    FlxTween.tween(heart.scale, {x: 0}, 0.5, {
    ...
    nameText = new FlxText(40, 120, 300, "名称", 48);
    nameText.setFormat(Paths.font("HanYiCaiYunTiJian.ttf"), 29, FlxColor.BLACK, "left");
    nameText.scrollFactor.set(0, 0);
    nameText.antialiasing = Options.antialiasing;
    add(nameText);

    socialText = new FlxText(40, 160, 380, "社交网站:", 20);
    socialText.setFormat(Paths.font("HanYiCaiYunTiJian.ttf"), 35, 0xFF000000, "left");
    socialText.scrollFactor.set(0, 0);
    socialText.antialiasing = Options.antialiasing;
    add(socialText);
    
    roleText = new FlxText(40, 280, 380, "角色", 24);
    roleText.setFormat(Paths.font("HanYiCaiYunTiJian.ttf"), 25, 0xFF000000, "left");
    roleText.scrollFactor.set(0, 0);
    roleText.antialiasing = Options.antialiasing;
    add(roleText);

    descText = new FlxText(40, 310, 290, "描述", 20);
    descText.setFormat(Paths.font("HanYiCaiYunTiJian.ttf"), 20, 0xFF000000, "left");
    descText.scrollFactor.set(0, 0);
    descText.antialiasing = Options.antialiasing;
    add(descText);

    quoteText = new FlxText(40, 340, 290, '"留言"', 20);
    quoteText.setFormat(Paths.font("HanYiCaiYunTiJian.ttf"), 29, FlxColor.BLACK, "left");
    ...
    backButton = new FlxText(FlxG.width - 200, FlxG.height - 85, 180, "返回", 64);
    backButton.setFormat(Paths.font("HanYiCaiYunTiJian.ttf"), 64, FlxColor.BLACK, "center");
