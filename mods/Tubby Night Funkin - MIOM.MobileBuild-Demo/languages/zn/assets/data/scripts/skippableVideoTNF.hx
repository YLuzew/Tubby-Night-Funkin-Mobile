...
function startVideo(name:String, ?leCallback:Void->Void, ?ext:String, ?usePath:Bool) {
    ...
    // 创建右上角跳过提示文字（英文）
    skipText = new FunkinText(0, 0, 0, '跳过:还剩 $clicksRemaining 次点击', 36);
    skipText.setFormat(Paths.font("HanYiCaiYunTiJian.ttf"), 36, FlxColor.WHITE, "right", FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
function updateSkipText() {
    if (skipText != null) {
        skipText.text = '跳过:还剩 $clicksRemaining 次点击';
        skipText.x = FlxG.width - skipText.width - 20;
        skipText.y = 20;
    }
}
