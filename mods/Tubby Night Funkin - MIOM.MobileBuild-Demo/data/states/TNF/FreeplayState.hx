...
function create() {
    ...
    accuracyText = new FlxText(FlxG.width, 10, 400, "Accuracy: 0% (?)", 48);
    accuracyText.setFormat(Paths.font("Tardling-Outline.ttf"), 32, FlxColor.WHITE, "right", FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
    ...
    backButton = new FlxText(20, FlxG.height - 70, 0, "Back", 48);
    backButton.setFormat(Paths.font("Tardling-Outline.ttf"), 48, FlxColor.WHITE, "left");
...
function update(elapsed:Float) {
    ...
    if (songUnlocked[curSelected] && songScored[curSelected])
        newDisplay = "Accuracy: " + Math.floor(lerpAccuracy) + "% (" + accuracyRank + ")";
    else if (songUnlocked[curSelected])
        newDisplay = "Accuracy: 0% (?)";
    else
        newDisplay = "LOCKED";
...
function addCard(songIdx:Int) {
    ...
    songText.setFormat(Paths.font("Tardling-Outline.ttf"), 75, FlxColor.WHITE, "center");
...
function updateScore() {
    if (songs.length == 0 || curSelected < 0 || curSelected >= songs.length) {
        ...
        accuracyRank = "N/A"; intendedAccuracy = 0; return;
    }
    if (!isVisible(curSelected) || !songUnlocked[curSelected]) {
        accuracyText.text = "LOCKED";
